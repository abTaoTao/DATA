...
#             # local_trans[cur_node] = np.mean([local_trans[cur_node]] + flow[net.nodes[cur_node]['type']], axis=0)
#             # for i, each in enumerate(local_trans[cur_node]):
#             #     if i not in net.nodes[cur_node]['neighbors']:
#             #         local_trans[cur_node][i] = 0
#             local_neighbors_list = [(v, d['time']) for u,v,d in net.edges(cur_node, data=True) if d['time']>=pre_time or d['time']=='always']

#             if local_neighbors_list == []:
#                 break

#             local_neighbors = {}
#             for neighbor in local_neighbors_list:
#                 try:
#                     local_neighbors[net.nodes[neighbor[0]]['type']].append(neighbor)
#                 except:
#                     local_neighbors[net.nodes[neighbor[0]]['type']] = [neighbor]

#             trans_type = []
#             for type_, each in enumerate(local_trans[cur_node]):
#                 try:
#                     local_neighbors[type_]
#                     trans_type.append(each)
#                 except:
#                     trans_type.append(0)

#             cur_type = unigram_sample(population=range(
#                 type_num), size=1, replace=True, weight=trans_type)[0]

#             local_trans[cur_node][cur_type] += 1

#             for type_ in flow:
#                 if not flow[type_].empty():
#                     flow[type_].get()
#             flow[net.nodes[cur_node]['type']].put(local_trans[cur_node])

#             cur_node, cur_time = random.choice(local_neighbors[cur_type])

#             if not cur_type == 'always':
#                 pre_time = cur_time
...
def walk_multiprocess(walk_times=10, max_num_workers=cpu_count()):
    ...
    #     for idx in range(mod):
    #         times_per_worker[idx] = times_per_worker[idx] + 1
    # assert sum(times_per_worker) == walk_times, 'workers allocating failed: %d != %d' % (sum(times_per_worker), walk_times)
...
filename = './datasets/taobao2/'
dataset = 'taobao'
out_filename = 'taobao.time.10.40.0.8.walk'
out_trans_filename = 'taobao.time.10.40.0.8.trans'
