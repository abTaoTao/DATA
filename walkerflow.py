...
def walk_multiprocess(walk_times=10, max_num_workers=cpu_count()):
    ...
    with ProcessPoolExecutor(max_workers=max_num_workers) as executor:
        for walks, trans, times in executor.map(walk_flow, args_list):
            sens.extend(walks)
...
filename = './datasets/dblp2/'
dataset = 'dblp'
out_filename = 'dblp.flow.5.40.walk'
out_trans_filename = 'dblp.flow.5.40.trans'
out_time_file = 'dblp.flow.5.40.times'
