...
def walk_flow(arg_list=(1, 0)):
    ...
    for root_node in root_nodes:
        ...
        for t in range(walk_length):
            ...
            neighbors_t = [e for e in net.nodes[cur_node]['neighbors_time'] if e[2] == next_type]
            if len(neighbors_t) == 0:
                neighbors_t = [e for e in net.nodes[cur_node]['neighbors_time']]
            next_node, now_time, _ = random.choice(neighbors_t)
            if now_time != 'always':
                pre_time = now_time
...
def random_walk(arg_list=(1, 0)):
    ...
    for root_node in root_nodes:
        ...
        for t in range(walk_length):
            ...
            neighbors_t = [e for e in net.nodes[cur_node]['neighbors_time']]
            if random.random() >= alpha:
                next_node, now_time, next_type = random.choice(neighbors_t)
            else:
                next_node = walk_instance[0]
                _, now_time, next_type = net.nodes[next_node]['neighbors_time']
...
def walk_multiprocess(walk_times=10, max_num_workers=cpu_count()):
    ...
    with ProcessPoolExecutor(max_workers=max_num_workers) as executor:
        for walks, trans, times in executor.map(random_walk, args_list):
            sens.extend(walks)
...
filename = './datasets/aminer2/'
dataset = 'Aminer'
out_filename = 'Aminer.random.5.40.walk'
out_trans_filename = 'Aminer.random.5.40.trans'
out_time_file = 'Aminer.random.5.40.times'
